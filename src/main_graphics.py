...
def GettingInput(parent_windows, string):
    layout = [
        ...
        [
            PySimpleGUI.Input("", size=(25, 1), enable_events=True, expand_x=True, key="-OFFSET-"),
        ],
        [
...
def RewriteOutputFile(output):
    ...
    window = PySimpleGUI.Window("Do you want to rewrite the output file?", layout)
    event, values = window.Read()
...
def SimilarCiphers(version):
    ...
    while (current_window >= 10):
        if current_window == 10:
            ...
            while source == "" and current_window == 10:
                if (version == Layouts.IMAGE.value):
                    ...
                    source, current_window = GettingFilePath(parent_windows, string_source)
            if current_window == 10:
        ...
        if current_window == 11:
            ...
            while key == "":
                key, current_window = GettingFilePath(parent_windows, string_key)
                if current_window != 10:
                    break
            if current_window == 10:
        ...
        if current_window == 12:
            ...
            while output == "":
                if (version == Layouts.IMAGE.value):
                    ...
                    output, current_window = GettingInput(parent_windows, string_output)
                else:
                    string_output = "Choose the output file, please"
                    output, current_window = GettingFilePath(parent_windows, string_output)
            if current_window != 10:
                if (current_window != 11):
                    break
        ...
        if current_window == 2:
            ...
            while (current_window >= 10):
                ...
                if current_window == 11:
                    ...
                    offset, current_window = GettingInput(parent_windows, string_offset)
                    if offset == "":
                        offset = 0
                    if current_window == 11:
                        parent_windows.append(current_window)
                ...
                if current_window == 12:
                    ...
                    with open(source) as inp:
                        for string in inp:
                            Cryptographer.CaesarCipher(string, int(offset), output)
                    current_window = -1
                    break
        if current_window == 3:
