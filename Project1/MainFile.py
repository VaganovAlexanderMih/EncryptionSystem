...
if __name__ == "__main__":
    for i in range (len(sys.argv)):
        if (skip_to >= i):
            continue
        if (sys.argv[i] == "--help"):
            print("""Write "--source" or "-s" to set source to the file that needed to be coded/decoded
...
if (decoder):
    ...
    if (cipher == "Vernam"):
        ...
        with open(source) as inp:
            for string in inp:
                Cryptographer.VernamCipher(string, key_path)
    elif (cipher == "Vigenere"):
        print("Insert KeyPath, please")
        key_path = str(input())
        f = open(key_path, "w")
        ...
        with open(source) as inp:
            for string in inp:
                Cryptographer.VigenereCipher(string, key_path)
    elif (cipher == "Caesar"):
        print("Input the offset, please")
        offset = int(input())
        with open(source) as inp:
            for string in inp:
                Cryptographer.CaesarCipher(string, offset)
